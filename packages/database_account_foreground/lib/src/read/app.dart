...
class DaoReadApp extends DatabaseAccessor<AccountForegroundDatabase> with _$DaoReadAppMixin {
  ...
  Stream<T?> _watchColumnInitialSync<T extends Object>(T? Function(InitialSyncData) extractColumn) {
    return (select(initialSync)..where((t) => t.id.equals(SingleRowTable.ID.value)))
      .map(extractColumn)
      .watchSingleOrNull();
  }
  ...
  Stream<T?> _watchColumnInitialSetupSkipped<T extends Object>(T? Function(InitialSetupSkippedData) extractColumn) {
    return (select(initialSetupSkipped)..where((t) => t.id.equals(SingleRowTable.ID.value)))
      .map(extractColumn)
      .watchSingleOrNull();
  }
  ...
  Stream<T?> _watchColumnProfileFilterFavorite<T extends Object>(T? Function(ProfileFilterFavorite) extractColumn) {
    return (select(profileFilterFavorites)..where((t) => t.id.equals(SingleRowTable.ID.value)))
      .map(extractColumn)
      .watchSingleOrNull();
  }

  Stream<bool?> watchShowAdvancedFilters() {
    return (select(showAdvancedProfileFilters)
