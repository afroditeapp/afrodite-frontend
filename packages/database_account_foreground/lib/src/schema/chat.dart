...
class MyKeyPair extends SingleRowTable {
  BlobColumn get privateKeyData => blob().map(const NullAwareTypeConverter.wrap(PrivateKeyBytesConverter())).nullable()();
  BlobColumn get publicKeyData => blob().map(const NullAwareTypeConverter.wrap(PublicKeyBytesConverter())).nullable()();
