...
class ProfileVersion {
  ...
  @override
  String toString() => 'ProfileVersion[v=$v]';
  ...
  static ProfileVersion? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'v'), 'Required key "ProfileVersion[v]" is missing from JSON.');
        assert(json[r'v'] != null, 'Required key "ProfileVersion[v]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<ProfileVersion>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = ProfileVersion.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
