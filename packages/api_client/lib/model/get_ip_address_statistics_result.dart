...
class GetIpAddressStatisticsResult {
  ...
  static GetIpAddressStatisticsResult? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'values'), 'Required key "GetIpAddressStatisticsResult[values]" is missing from JSON.');
        assert(json[r'values'] != null, 'Required key "GetIpAddressStatisticsResult[values]" has a null value in JSON.');
        return true;
      }());
