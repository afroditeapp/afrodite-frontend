...
class ApiUsageStatistics {
  ...
  static ApiUsageStatistics? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'name'), 'Required key "ApiUsageStatistics[name]" is missing from JSON.');
        assert(json[r'name'] != null, 'Required key "ApiUsageStatistics[name]" has a null value in JSON.');
        assert(json.containsKey(r'values'), 'Required key "ApiUsageStatistics[values]" is missing from JSON.');
        assert(json[r'values'] != null, 'Required key "ApiUsageStatistics[values]" has a null value in JSON.');
        return true;
      }());
