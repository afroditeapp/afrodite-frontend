...
class PushNotificationDeviceToken {
  ...
  static PushNotificationDeviceToken? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'token'), 'Required key "PushNotificationDeviceToken[token]" is missing from JSON.');
        assert(json[r'token'] != null, 'Required key "PushNotificationDeviceToken[token]" has a null value in JSON.');
        return true;
      }());
