...
class DailyLikesLeftSyncVersion {
  ...
  static DailyLikesLeftSyncVersion? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'version'), 'Required key "DailyLikesLeftSyncVersion[version]" is missing from JSON.');
        assert(json[r'version'] != null, 'Required key "DailyLikesLeftSyncVersion[version]" has a null value in JSON.');
        return true;
      }());
