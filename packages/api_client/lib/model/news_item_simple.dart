...
class NewsItemSimple {
  ...
  static NewsItemSimple? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'id'), 'Required key "NewsItemSimple[id]" is missing from JSON.');
        assert(json[r'id'] != null, 'Required key "NewsItemSimple[id]" has a null value in JSON.');
        return true;
      }());
