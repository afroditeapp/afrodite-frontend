...
class PushNotificationEncryptionKey {
  ...
  static PushNotificationEncryptionKey? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'key'), 'Required key "PushNotificationEncryptionKey[key]" is missing from JSON.');
        assert(json[r'key'] != null, 'Required key "PushNotificationEncryptionKey[key]" has a null value in JSON.');
        return true;
      }());
