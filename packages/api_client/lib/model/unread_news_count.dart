...
class UnreadNewsCount {
  ...
  static UnreadNewsCount? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'c'), 'Required key "UnreadNewsCount[c]" is missing from JSON.');
        assert(json[r'c'] != null, 'Required key "UnreadNewsCount[c]" has a null value in JSON.');
        return true;
      }());
