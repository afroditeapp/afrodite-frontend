...
class SendLike {
  ...
  static SendLike? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'account_id'), 'Required key "SendLike[account_id]" is missing from JSON.');
        assert(json[r'account_id'] != null, 'Required key "SendLike[account_id]" has a null value in JSON.');
        return true;
      }());
