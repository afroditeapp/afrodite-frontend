...
class PendingAppNotificationToDelete {
  ...
  static PendingAppNotificationToDelete? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'notification_type'), 'Required key "PendingAppNotificationToDelete[notification_type]" is missing from JSON.');
        assert(json[r'notification_type'] != null, 'Required key "PendingAppNotificationToDelete[notification_type]" has a null value in JSON.');
        return true;
      }());
