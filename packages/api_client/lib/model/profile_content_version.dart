...
class ProfileContentVersion {
  ...
  static ProfileContentVersion? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'v'), 'Required key "ProfileContentVersion[v]" is missing from JSON.');
        assert(json[r'v'] != null, 'Required key "ProfileContentVersion[v]" has a null value in JSON.');
        return true;
      }());
