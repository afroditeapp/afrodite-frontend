...
class AccessibleAccount {
  ...
  static AccessibleAccount? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'aid'), 'Required key "AccessibleAccount[aid]" is missing from JSON.');
        assert(json[r'aid'] != null, 'Required key "AccessibleAccount[aid]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<AccessibleAccount>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = AccessibleAccount.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
