...
class SetProfileName {
  ...
  static SetProfileName? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'account'), 'Required key "SetProfileName[account]" is missing from JSON.');
        assert(json[r'account'] != null, 'Required key "SetProfileName[account]" has a null value in JSON.');
        return true;
      }());
