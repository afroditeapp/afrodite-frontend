...
class GetApiUsageStatisticsSettings {
  ...
  static GetApiUsageStatisticsSettings? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'account'), 'Required key "GetApiUsageStatisticsSettings[account]" is missing from JSON.');
        assert(json[r'account'] != null, 'Required key "GetApiUsageStatisticsSettings[account]" has a null value in JSON.');
        return true;
      }());
