...
class ImageProcessingDynamicConfig {
  ...
  static ImageProcessingDynamicConfig? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'nsfw_thresholds'), 'Required key "ImageProcessingDynamicConfig[nsfw_thresholds]" is missing from JSON.');
        assert(json[r'nsfw_thresholds'] != null, 'Required key "ImageProcessingDynamicConfig[nsfw_thresholds]" has a null value in JSON.');
        return true;
      }());
