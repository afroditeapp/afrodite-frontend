...
class SystemInfo {
  ...
  static SystemInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'info'), 'Required key "SystemInfo[info]" is missing from JSON.');
        assert(json[r'info'] != null, 'Required key "SystemInfo[info]" has a null value in JSON.');
        return true;
      }());
