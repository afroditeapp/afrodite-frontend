...
class FavoriteProfilesPage {
  ...
  static FavoriteProfilesPage? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'profiles'), 'Required key "FavoriteProfilesPage[profiles]" is missing from JSON.');
        assert(json[r'profiles'] != null, 'Required key "FavoriteProfilesPage[profiles]" has a null value in JSON.');
        return true;
      }());
