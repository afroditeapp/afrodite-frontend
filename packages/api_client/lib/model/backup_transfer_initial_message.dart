...
class BackupTransferInitialMessage {
  ...
  static BackupTransferInitialMessage? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'role'), 'Required key "BackupTransferInitialMessage[role]" is missing from JSON.');
        assert(json[r'role'] != null, 'Required key "BackupTransferInitialMessage[role]" has a null value in JSON.');
        return true;
      }());
