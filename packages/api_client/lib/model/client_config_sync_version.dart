...
class ClientConfigSyncVersion {
  ...
  static ClientConfigSyncVersion? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'version'), 'Required key "ClientConfigSyncVersion[version]" is missing from JSON.');
        assert(json[r'version'] != null, 'Required key "ClientConfigSyncVersion[version]" has a null value in JSON.');
        return true;
      }());
