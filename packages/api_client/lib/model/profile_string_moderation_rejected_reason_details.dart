...
class ProfileStringModerationRejectedReasonDetails {
  ...
  static ProfileStringModerationRejectedReasonDetails? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'value'), 'Required key "ProfileStringModerationRejectedReasonDetails[value]" is missing from JSON.');
        assert(json[r'value'] != null, 'Required key "ProfileStringModerationRejectedReasonDetails[value]" has a null value in JSON.');
        return true;
      }());
