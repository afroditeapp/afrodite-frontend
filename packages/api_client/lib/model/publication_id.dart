...
class PublicationId {
  ...
  static PublicationId? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'id'), 'Required key "PublicationId[id]" is missing from JSON.');
        assert(json[r'id'] != null, 'Required key "PublicationId[id]" has a null value in JSON.');
        return true;
      }());
