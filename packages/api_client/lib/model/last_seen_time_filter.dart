...
class LastSeenTimeFilter {
  ...
  static LastSeenTimeFilter? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'value'), 'Required key "LastSeenTimeFilter[value]" is missing from JSON.');
        assert(json[r'value'] != null, 'Required key "LastSeenTimeFilter[value]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<LastSeenTimeFilter>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = LastSeenTimeFilter.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
