...
class ManualAssociationMembershipRegistry {
  ...
  static ManualAssociationMembershipRegistry? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'registry'), 'Required key "ManualAssociationMembershipRegistry[registry]" is missing from JSON.');
        assert(json[r'registry'] != null, 'Required key "ManualAssociationMembershipRegistry[registry]" has a null value in JSON.');
        return true;
      }());
