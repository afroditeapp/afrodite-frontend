...
class ClientConfig {
  ...
  static ClientConfig? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'sync_version'), 'Required key "ClientConfig[sync_version]" is missing from JSON.');
        assert(json[r'sync_version'] != null, 'Required key "ClientConfig[sync_version]" has a null value in JSON.');
        return true;
      }());
