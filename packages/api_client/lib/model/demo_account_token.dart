...
class DemoAccountToken {
  ...
  static DemoAccountToken? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'token'), 'Required key "DemoAccountToken[token]" is missing from JSON.');
        assert(json[r'token'] != null, 'Required key "DemoAccountToken[token]" has a null value in JSON.');
        return true;
      }());
