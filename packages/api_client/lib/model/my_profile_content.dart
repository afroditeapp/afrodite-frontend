...
class MyProfileContent {
  ...
  static MyProfileContent? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'content'), 'Required key "MyProfileContent[content]" is missing from JSON.');
        assert(json[r'content'] != null, 'Required key "MyProfileContent[content]" has a null value in JSON.');
        assert(json.containsKey(r'verification_status'), 'Required key "MyProfileContent[verification_status]" is missing from JSON.');
        assert(json[r'verification_status'] != null, 'Required key "MyProfileContent[verification_status]" has a null value in JSON.');
        return true;
      }());
