...
class BackupTransferTargetData {
  ...
  static BackupTransferTargetData? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'target_data'), 'Required key "BackupTransferTargetData[target_data]" is missing from JSON.');
        assert(json[r'target_data'] != null, 'Required key "BackupTransferTargetData[target_data]" has a null value in JSON.');
        return true;
      }());
