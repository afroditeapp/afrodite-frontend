...
class JitsiMeetUrl {
  ...
  static JitsiMeetUrl? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'url'), 'Required key "JitsiMeetUrl[url]" is missing from JSON.');
        assert(json[r'url'] != null, 'Required key "JitsiMeetUrl[url]" has a null value in JSON.');
        return true;
      }());
