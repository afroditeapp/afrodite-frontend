...
class NewReceivedLikesCount {
  ...
  static NewReceivedLikesCount? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'c'), 'Required key "NewReceivedLikesCount[c]" is missing from JSON.');
        assert(json[r'c'] != null, 'Required key "NewReceivedLikesCount[c]" has a null value in JSON.');
        return true;
      }());
