...
class RefreshToken {
  ...
  static RefreshToken? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'token'), 'Required key "RefreshToken[token]" is missing from JSON.');
        assert(json[r'token'] != null, 'Required key "RefreshToken[token]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<RefreshToken>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = RefreshToken.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
