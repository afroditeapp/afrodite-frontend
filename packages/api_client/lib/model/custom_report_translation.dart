...
class CustomReportTranslation {
  ...
  static CustomReportTranslation? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'key'), 'Required key "CustomReportTranslation[key]" is missing from JSON.');
        assert(json[r'key'] != null, 'Required key "CustomReportTranslation[key]" has a null value in JSON.');
        assert(json.containsKey(r'name'), 'Required key "CustomReportTranslation[name]" is missing from JSON.');
        assert(json[r'name'] != null, 'Required key "CustomReportTranslation[name]" has a null value in JSON.');
        return true;
      }());
