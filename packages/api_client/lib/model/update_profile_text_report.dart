...
class UpdateProfileTextReport {
  ...
  static UpdateProfileTextReport? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'profile_text'), 'Required key "UpdateProfileTextReport[profile_text]" is missing from JSON.');
        assert(json[r'profile_text'] != null, 'Required key "UpdateProfileTextReport[profile_text]" has a null value in JSON.');
        assert(json.containsKey(r'target'), 'Required key "UpdateProfileTextReport[target]" is missing from JSON.');
        assert(json[r'target'] != null, 'Required key "UpdateProfileTextReport[target]" has a null value in JSON.');
        return true;
      }());
