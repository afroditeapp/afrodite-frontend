...
class ProfileStringModerationInfo {
  ...
  static ProfileStringModerationInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'state'), 'Required key "ProfileStringModerationInfo[state]" is missing from JSON.');
        assert(json[r'state'] != null, 'Required key "ProfileStringModerationInfo[state]" has a null value in JSON.');
        return true;
      }());
