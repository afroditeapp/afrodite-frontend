...
class SentBlocksPage {
  ...
  static SentBlocksPage? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'profiles'), 'Required key "SentBlocksPage[profiles]" is missing from JSON.');
        assert(json[r'profiles'] != null, 'Required key "SentBlocksPage[profiles]" has a null value in JSON.');
        return true;
      }());
