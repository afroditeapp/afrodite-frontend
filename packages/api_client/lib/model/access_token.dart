...
class AccessToken {
  ...
  static AccessToken? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'token'), 'Required key "AccessToken[token]" is missing from JSON.');
        assert(json[r'token'] != null, 'Required key "AccessToken[token]" has a null value in JSON.');
        return true;
      }());
