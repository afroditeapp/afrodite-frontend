...
class ReceivedLikesPage {
  ...
  static ReceivedLikesPage? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'l'), 'Required key "ReceivedLikesPage[l]" is missing from JSON.');
        assert(json[r'l'] != null, 'Required key "ReceivedLikesPage[l]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<ReceivedLikesPage>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = ReceivedLikesPage.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
