...
class AccountBanReasonCategory {
  ...
  static AccountBanReasonCategory? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'value'), 'Required key "AccountBanReasonCategory[value]" is missing from JSON.');
        assert(json[r'value'] != null, 'Required key "AccountBanReasonCategory[value]" has a null value in JSON.');
        return true;
      }());
