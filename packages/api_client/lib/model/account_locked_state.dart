...
class AccountLockedState {
  ...
  static AccountLockedState? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'locked'), 'Required key "AccountLockedState[locked]" is missing from JSON.');
        assert(json[r'locked'] != null, 'Required key "AccountLockedState[locked]" has a null value in JSON.');
        return true;
      }());
