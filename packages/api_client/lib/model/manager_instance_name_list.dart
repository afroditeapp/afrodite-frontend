...
class ManagerInstanceNameList {
  ...
  static ManagerInstanceNameList? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'names'), 'Required key "ManagerInstanceNameList[names]" is missing from JSON.');
        assert(json[r'names'] != null, 'Required key "ManagerInstanceNameList[names]" has a null value in JSON.');
        return true;
      }());
