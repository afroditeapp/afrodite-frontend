//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

library openapi.api;

import 'dart:async';
import 'dart:convert';
import 'dart:io';

import 'package:collection/collection.dart';
import 'package:http/http.dart';
import 'package:intl/intl.dart';
import 'package:meta/meta.dart';

part 'api_client.dart';
part 'api_helper.dart';
part 'api_exception.dart';
part 'auth/authentication.dart';
part 'auth/api_key_auth.dart';
part 'auth/oauth.dart';
part 'auth/http_basic_auth.dart';
part 'auth/http_bearer_auth.dart';

part 'api/account_api.dart';
part 'api/account_admin_api.dart';
part 'api/account_bot_api.dart';
part 'api/chat_api.dart';
part 'api/chat_transfer_api.dart';
part 'api/common_api.dart';
part 'api/common_admin_api.dart';
part 'api/media_api.dart';
part 'api/media_admin_api.dart';
part 'api/profile_api.dart';
part 'api/profile_admin_api.dart';

part 'model/accept_or_reject.dart';
part 'model/access_token.dart';
part 'model/accessible_account.dart';
part 'model/account.dart';
part 'model/account_app_notification_settings.dart';
part 'model/account_ban_reason_category.dart';
part 'model/account_ban_reason_details.dart';
part 'model/account_banned_admin_type.dart';
part 'model/account_content.dart';
part 'model/account_id.dart';
part 'model/account_id_db_value.dart';
part 'model/account_locked_state.dart';
part 'model/account_login_platforms.dart';
part 'model/account_registration_platforms.dart';
part 'model/account_state_container.dart';
part 'model/account_sync_version.dart';
part 'model/account_verification_config.dart';
part 'model/account_verification_error_flags_value.dart';
part 'model/account_verification_methods_config.dart';
part 'model/account_verification_platforms.dart';
part 'model/account_verification_queue_admin_item.dart';
part 'model/account_verification_queue_item.dart';
part 'model/account_verification_queue_status.dart';
part 'model/account_verification_scope.dart';
part 'model/account_verification_scopes_config.dart';
part 'model/add_favorite_profile_result.dart';
part 'model/add_public_key_result.dart';
part 'model/admin_bot_account_verification_config.dart';
part 'model/admin_bot_base_llm_config.dart';
part 'model/admin_bot_config.dart';
part 'model/admin_bot_content_moderation_config.dart';
part 'model/admin_bot_content_moderation_llm_config.dart';
part 'model/admin_bot_face_verification_config.dart';
part 'model/admin_bot_face_verification_llm_config.dart';
part 'model/admin_bot_moderation_base_llm_config.dart';
part 'model/admin_bot_nsfw_detection_config.dart';
part 'model/admin_bot_profile_string_moderation_config.dart';
part 'model/admin_bot_report_processing_config.dart';
part 'model/admin_bot_report_processing_messages_config.dart';
part 'model/admin_bot_report_processing_messages_llm_config.dart';
part 'model/admin_bot_report_processing_profile_content_config.dart';
part 'model/admin_bot_report_processing_profile_content_llm_config.dart';
part 'model/admin_bot_report_processing_profile_string_config.dart';
part 'model/admin_bot_report_processing_profile_string_llm_config.dart';
part 'model/admin_bot_security_content_verification_config.dart';
part 'model/admin_bot_security_content_verification_llm_config.dart';
part 'model/admin_bot_string_moderation_llm_config.dart';
part 'model/admin_bot_string_report_base_llm_config.dart';
part 'model/admin_info.dart';
part 'model/admin_notification.dart';
part 'model/admin_notification_settings.dart';
part 'model/age_verification_config.dart';
part 'model/age_verification_method.dart';
part 'model/age_verification_methods_config.dart';
part 'model/age_verification_platforms.dart';
part 'model/api_usage_count.dart';
part 'model/api_usage_statistics.dart';
part 'model/app_attestation.dart';
part 'model/app_attestation_type_number.dart';
part 'model/association_config.dart';
part 'model/association_member.dart';
part 'model/association_members_page.dart';
part 'model/association_membership.dart';
part 'model/attribute.dart';
part 'model/attribute_hash.dart';
part 'model/attribute_mode.dart';
part 'model/attribute_order_mode.dart';
part 'model/attribute_value.dart';
part 'model/attribute_value_order_mode.dart';
part 'model/attribution_config.dart';
part 'model/auth_pair.dart';
part 'model/automatic_banning_day_count_config.dart';
part 'model/automatic_banning_expected_llm_responses_config.dart';
part 'model/automatic_profile_search_iterator_session_id.dart';
part 'model/automatic_profile_search_settings.dart';
part 'model/backup_transfer_byte_count.dart';
part 'model/backup_transfer_client_role.dart';
part 'model/backup_transfer_initial_message.dart';
part 'model/backup_transfer_target_data.dart';
part 'model/banner_platform.dart';
part 'model/banner_visibility.dart';
part 'model/boolean_setting.dart';
part 'model/bot_account.dart';
part 'model/bot_config.dart';
part 'model/chat_app_notification_settings.dart';
part 'model/chat_config.dart';
part 'model/chat_email_notification_settings.dart';
part 'model/chat_message_report.dart';
part 'model/chat_message_report_data.dart';
part 'model/chat_privacy_settings.dart';
part 'model/check_online_status_config.dart';
part 'model/client_config.dart';
part 'model/client_config_sync_version.dart';
part 'model/client_features_config.dart';
part 'model/client_features_config_hash.dart';
part 'model/client_info.dart';
part 'model/client_language.dart';
part 'model/client_message_type.dart';
part 'model/client_type.dart';
part 'model/client_version.dart';
part 'model/client_version_count.dart';
part 'model/client_version_statistics.dart';
part 'model/command_output.dart';
part 'model/connection_statistics.dart';
part 'model/content_id.dart';
part 'model/content_info.dart';
part 'model/content_info_detailed.dart';
part 'model/content_moderation_state.dart';
part 'model/content_processing_state.dart';
part 'model/content_processing_state_type.dart';
part 'model/content_slot.dart';
part 'model/conversation_id.dart';
part 'model/current_account_interaction_state.dart';
part 'model/custom_email.dart';
part 'model/custom_email_id.dart';
part 'model/custom_email_target_group.dart';
part 'model/custom_email_translation.dart';
part 'model/custom_report.dart';
part 'model/custom_report_language.dart';
part 'model/custom_report_translation.dart';
part 'model/custom_report_type.dart';
part 'model/custom_reports_config.dart';
part 'model/custom_reports_config_hash.dart';
part 'model/custom_reports_order_mode.dart';
part 'model/daily_likes_config.dart';
part 'model/daily_likes_left.dart';
part 'model/daily_likes_left_sync_version.dart';
part 'model/data_export_name.dart';
part 'model/data_export_state.dart';
part 'model/data_export_state_type.dart';
part 'model/data_export_type.dart';
part 'model/delivery_info_type.dart';
part 'model/demo_account_login_credentials.dart';
part 'model/demo_account_login_result.dart';
part 'model/demo_account_login_to_account.dart';
part 'model/demo_account_register_account_result.dart';
part 'model/demo_account_token.dart';
part 'model/dynamic_client_features_config.dart';
part 'model/dynamic_client_features_config_hash.dart';
part 'model/dynamic_server_config.dart';
part 'model/edit_verification_profile_age_range.dart';
part 'model/edit_verification_profile_name.dart';
part 'model/edit_verification_security_content.dart';
part 'model/edit_verification_values.dart';
part 'model/email_address_state.dart';
part 'model/email_address_state_admin.dart';
part 'model/email_login.dart';
part 'model/email_login_token.dart';
part 'model/email_registration_domain_lists.dart';
part 'model/email_registration_platforms.dart';
part 'model/favorite_profiles_page.dart';
part 'model/features_config.dart';
part 'model/first_image_config.dart';
part 'model/get_account_ban_time_result.dart';
part 'model/get_account_deletion_request_result.dart';
part 'model/get_account_id_from_email_result.dart';
part 'model/get_account_login_session_info.dart';
part 'model/get_account_verification_queue_next_item_result.dart';
part 'model/get_all_admins_result.dart';
part 'model/get_api_usage_statistics_result.dart';
part 'model/get_api_usage_statistics_settings.dart';
part 'model/get_association_member.dart';
part 'model/get_association_members_only_info.dart';
part 'model/get_association_members_page.dart';
part 'model/get_association_membership.dart';
part 'model/get_bots_result.dart';
part 'model/get_chat_message_reports.dart';
part 'model/get_client_features_config_result.dart';
part 'model/get_client_language.dart';
part 'model/get_client_version_statistics_result.dart';
part 'model/get_client_version_statistics_settings.dart';
part 'model/get_content_processing_state.dart';
part 'model/get_conversation_id.dart';
part 'model/get_custom_email_config.dart';
part 'model/get_custom_reports_config_result.dart';
part 'model/get_dynamic_client_features_config_result.dart';
part 'model/get_initial_profile_age_result.dart';
part 'model/get_ip_address_statistics_result.dart';
part 'model/get_ip_address_statistics_settings.dart';
part 'model/get_ip_country_statistics_result.dart';
part 'model/get_ip_country_statistics_settings.dart';
part 'model/get_latest_public_key_id.dart';
part 'model/get_media_content_face_verified_null_list.dart';
part 'model/get_media_content_result.dart';
part 'model/get_my_profile_result.dart';
part 'model/get_news_item_result.dart';
part 'model/get_private_public_key_info.dart';
part 'model/get_profile_age_and_name.dart';
part 'model/get_profile_content_result.dart';
part 'model/get_profile_filters.dart';
part 'model/get_profile_result.dart';
part 'model/get_profile_statistics_history_result.dart';
part 'model/get_profile_statistics_result.dart';
part 'model/get_profile_string_state.dart';
part 'model/get_push_notification_info.dart';
part 'model/get_report_list.dart';
part 'model/get_report_queue_page.dart';
part 'model/get_sent_message.dart';
part 'model/image_processing_dynamic_config.dart';
part 'model/image_processing_warnings.dart';
part 'model/info_banner.dart';
part 'model/info_banner_mode.dart';
part 'model/info_banner_url_button.dart';
part 'model/info_banners_config.dart';
part 'model/init_email_change.dart';
part 'model/init_email_change_admin.dart';
part 'model/init_email_change_result.dart';
part 'model/initial_profile_age.dart';
part 'model/ip_address_info.dart';
part 'model/ip_country_statistics.dart';
part 'model/ip_country_statistics_type.dart';
part 'model/ip_country_statistics_value.dart';
part 'model/jitsi_meet_url.dart';
part 'model/language.dart';
part 'model/last_seen_time_filter.dart';
part 'model/latest_seen_message_info.dart';
part 'model/latest_seen_message_info_list.dart';
part 'model/likes_config.dart';
part 'model/limited_action_status.dart';
part 'model/location.dart';
part 'model/login_result.dart';
part 'model/login_session_info.dart';
part 'model/maintenance_task.dart';
part 'model/manager_instance_name_list.dart';
part 'model/manual_association_membership_registry.dart';
part 'model/manual_association_membership_registry_input.dart';
part 'model/manual_server_maintenance_info_for_another_server.dart';
part 'model/map_bounds.dart';
part 'model/map_config.dart';
part 'model/map_coordinate.dart';
part 'model/map_zoom.dart';
part 'model/mark_received_likes_viewed.dart';
part 'model/match_id.dart';
part 'model/matches_iterator_state.dart';
part 'model/matches_page.dart';
part 'model/max_distance_km.dart';
part 'model/media_app_notification_settings.dart';
part 'model/media_content_face_verified_null_by_account.dart';
part 'model/media_content_moderation_queue_page.dart';
part 'model/media_content_moderation_queue_type.dart';
part 'model/media_content_moderation_rejected_reason_category.dart';
part 'model/media_content_moderation_rejected_reason_details.dart';
part 'model/media_content_moderation_type.dart';
part 'model/media_content_pending_moderation.dart';
part 'model/media_content_sync_version.dart';
part 'model/media_content_type.dart';
part 'model/media_content_upload_type.dart';
part 'model/media_verification_status.dart';
part 'model/membership_type.dart';
part 'model/message_delivery_info.dart';
part 'model/message_delivery_info_id_list.dart';
part 'model/message_delivery_info_list.dart';
part 'model/message_id.dart';
part 'model/message_number.dart';
part 'model/min_distance_km.dart';
part 'model/moderation_action.dart';
part 'model/my_content_info.dart';
part 'model/my_profile_content.dart';
part 'model/new_received_likes_count.dart';
part 'model/new_received_likes_count_result.dart';
part 'model/news_config.dart';
part 'model/news_id.dart';
part 'model/news_item.dart';
part 'model/news_item_simple.dart';
part 'model/news_iterator_state.dart';
part 'model/news_page.dart';
part 'model/news_sync_version.dart';
part 'model/news_translation_version.dart';
part 'model/nsfw_detection_thresholds.dart';
part 'model/partial_profile_attributes_config.dart';
part 'model/pending_app_notification.dart';
part 'model/pending_app_notification_to_delete.dart';
part 'model/pending_app_notification_type.dart';
part 'model/pending_chat_notification.dart';
part 'model/pending_chat_notification_to_delete.dart';
part 'model/pending_message_acknowledgement_list.dart';
part 'model/pending_message_id.dart';
part 'model/perf_metric_query.dart';
part 'model/perf_metric_query_result.dart';
part 'model/perf_metric_value_area.dart';
part 'model/perf_metric_values.dart';
part 'model/permissions.dart';
part 'model/play_integrity_app_attestation.dart';
part 'model/post_account_verification_queue_item_result.dart';
part 'model/post_account_verification_queue_remove_next_item.dart';
part 'model/post_age_verification.dart';
part 'model/post_age_verification_result.dart';
part 'model/post_media_content_face_detected_value.dart';
part 'model/post_media_content_face_verified_value.dart';
part 'model/post_media_content_face_verified_value_item.dart';
part 'model/post_moderate_media_content.dart';
part 'model/post_moderate_profile_string.dart';
part 'model/post_profile_age_range_verified_value.dart';
part 'model/post_profile_name_verified_value.dart';
part 'model/post_security_content_verified_value.dart';
part 'model/post_start_data_export.dart';
part 'model/post_video_call_url_result.dart';
part 'model/predefined_banner.dart';
part 'model/process_report.dart';
part 'model/process_reports.dart';
part 'model/profile.dart';
part 'model/profile_age_counts.dart';
part 'model/profile_age_range_verification_admin_info.dart';
part 'model/profile_app_notification_settings.dart';
part 'model/profile_attribute_filter_value.dart';
part 'model/profile_attribute_filter_value_update.dart';
part 'model/profile_attribute_info.dart';
part 'model/profile_attribute_value.dart';
part 'model/profile_attribute_value_update.dart';
part 'model/profile_attributes_config_query.dart';
part 'model/profile_attributes_config_query_item.dart';
part 'model/profile_attributes_config_query_result.dart';
part 'model/profile_attributes_schema_export.dart';
part 'model/profile_config.dart';
part 'model/profile_content.dart';
part 'model/profile_content_version.dart';
part 'model/profile_created_time_filter.dart';
part 'model/profile_edited_time_filter.dart';
part 'model/profile_filters_update.dart';
part 'model/profile_iterator_page.dart';
part 'model/profile_iterator_page_item.dart';
part 'model/profile_iterator_page_value.dart';
part 'model/profile_iterator_session_id.dart';
part 'model/profile_link.dart';
part 'model/profile_name_verification_admin_info.dart';
part 'model/profile_page.dart';
part 'model/profile_privacy_settings.dart';
part 'model/profile_statistics_history_value.dart';
part 'model/profile_statistics_history_value_type.dart';
part 'model/profile_string_moderation_content_type.dart';
part 'model/profile_string_moderation_info.dart';
part 'model/profile_string_moderation_queue_page.dart';
part 'model/profile_string_moderation_queue_type.dart';
part 'model/profile_string_moderation_rejected_reason_category.dart';
part 'model/profile_string_moderation_rejected_reason_details.dart';
part 'model/profile_string_moderation_state.dart';
part 'model/profile_string_pending_moderation.dart';
part 'model/profile_sync_version.dart';
part 'model/profile_text_max_characters_filter.dart';
part 'model/profile_text_min_characters_filter.dart';
part 'model/profile_update.dart';
part 'model/profile_verification_status.dart';
part 'model/profile_verification_status_filter.dart';
part 'model/profile_version.dart';
part 'model/profile_visibility.dart';
part 'model/public_key_id.dart';
part 'model/publication_id.dart';
part 'model/push_notification_device_token.dart';
part 'model/push_notification_encryption_key.dart';
part 'model/push_notification_info_sync_version.dart';
part 'model/put_content_to_content_slot_result.dart';
part 'model/put_sign_in_with_apple.dart';
part 'model/put_sign_in_with_google.dart';
part 'model/put_sign_in_with_result.dart';
part 'model/received_like_id.dart';
part 'model/received_likes_iterator_state.dart';
part 'model/received_likes_page.dart';
part 'model/received_likes_page_item.dart';
part 'model/received_likes_sync_version.dart';
part 'model/refresh_token.dart';
part 'model/remote_bot_login.dart';
part 'model/remote_bot_password.dart';
part 'model/report_account_info.dart';
part 'model/report_chat_info.dart';
part 'model/report_chat_info_interaction_state.dart';
part 'model/report_content.dart';
part 'model/report_detailed.dart';
part 'model/report_detailed_info.dart';
part 'model/report_id.dart';
part 'model/report_iterator_mode.dart';
part 'model/report_iterator_query.dart';
part 'model/report_processing_state.dart';
part 'model/report_queue_type.dart';
part 'model/report_type.dart';
part 'model/request_email_login_token.dart';
part 'model/request_email_login_token_result.dart';
part 'model/reset_news_iterator_result.dart';
part 'model/reset_received_likes_iterator_result.dart';
part 'model/save_info_banners.dart';
part 'model/scheduled_task_status.dart';
part 'model/scheduled_task_type.dart';
part 'model/scheduled_task_type_value.dart';
part 'model/search_age_range.dart';
part 'model/search_groups.dart';
part 'model/security_content_admin_info.dart';
part 'model/seen_message.dart';
part 'model/send_custom_email.dart';
part 'model/send_like.dart';
part 'model/send_like_result.dart';
part 'model/send_message_result.dart';
part 'model/send_verify_email_message_result.dart';
part 'model/sent_blocks_page.dart';
part 'model/sent_message_id_list.dart';
part 'model/server_maintenance_status.dart';
part 'model/server_message_type.dart';
part 'model/server_version.dart';
part 'model/set_account_ban_state.dart';
part 'model/set_email_login_enabled.dart';
part 'model/set_initial_email.dart';
part 'model/set_profile_content.dart';
part 'model/set_profile_name.dart';
part 'model/sign_in_with_apple_info.dart';
part 'model/sign_in_with_google_info.dart';
part 'model/sign_in_with_login_info.dart';
part 'model/sign_in_with_state.dart';
part 'model/software_info.dart';
part 'model/software_update_state.dart';
part 'model/software_update_status.dart';
part 'model/statistics_profile_visibility.dart';
part 'model/string_resource.dart';
part 'model/sync_version.dart';
part 'model/system_info.dart';
part 'model/text_info_banner.dart';
part 'model/time_granularity.dart';
part 'model/translation.dart';
part 'model/typing_indicator_config.dart';
part 'model/unix_time.dart';
part 'model/unread_news_count.dart';
part 'model/unread_news_count_result.dart';
part 'model/unsigned_integer_attribute_config.dart';
part 'model/update_association_membership.dart';
part 'model/update_association_membership_type.dart';
part 'model/update_chat_message_reports.dart';
part 'model/update_custom_email.dart';
part 'model/update_custom_report_empty.dart';
part 'model/update_news_translation.dart';
part 'model/update_news_translation_result.dart';
part 'model/update_profile_attributes_schema.dart';
part 'model/update_profile_content_report.dart';
part 'model/update_profile_content_result.dart';
part 'model/update_profile_name_report.dart';
part 'model/update_profile_text_report.dart';
part 'model/update_report_result.dart';
part 'model/vapid_public_key.dart';
part 'model/verification_method.dart';


/// An [ApiClient] instance that uses the default values obtained from
/// the OpenAPI specification file.
var defaultApiClient = ApiClient();

const _delimiters = {'csv': ',', 'ssv': ' ', 'tsv': '\t', 'pipes': '|'};
const _dateEpochMarker = 'epoch';
const _deepEquality = DeepCollectionEquality();
final _dateFormatter = DateFormat('yyyy-MM-dd');
final _regList = RegExp(r'^List<(.*)>$');
final _regSet = RegExp(r'^Set<(.*)>$');
final _regMap = RegExp(r'^Map<String,(.*)>$');

bool _isEpochMarker(String? pattern) => pattern == _dateEpochMarker || pattern == '/$_dateEpochMarker/';
