...
class MatchesDatabaseIterator extends IteratorType {
  int currentIndex;
  final AccountDatabaseManager db;
  MatchesDatabaseIterator({this.currentIndex = 0, required this.db});
  ...
  Future<Result<List<ProfileEntry>, void>> nextList() async {
    const queryCount = 10;
    final profiles = await db.accountData((db) => db.daoProfileStates.getMatchesGridList(currentIndex, queryCount)).ok();
    if (profiles != null) {
