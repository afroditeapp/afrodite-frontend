...
class ReceivedLikesIteratorManager {
  ...
  ReceivedLikesIteratorManager(this.chat, this.media, this.accountBackgroundDb, this.db, this.connectionManager, this.currentUser) :
    _currentIterator = ReceivedLikesDatabaseIterator(db: db);
  ...
  Future<Result<List<ProfileEntry>, void>> _nextListRaw() async {
    ...
    if (nextList.isEmpty && _currentIterator is OnlineIterator) {
      _currentIterator = ReceivedLikesDatabaseIterator(
        db: db,
      );
    }
    return Ok(nextList);
