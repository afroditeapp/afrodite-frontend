...
class NewModerationRequestBloc extends Bloc<NewModerationRequestEvent, NewModerationRequestData>
    with ActionRunner {
  NewModerationRequestBloc() : super(NewModerationRequestData()) {
    ...
    on<AddImg>((data, emit) async {
      emit(
        state.copyWith(
          selectedImgs: state.selectedImgs.add(data.slot, data.img, data.faceDetected),
        ),
      );
    });
  }
