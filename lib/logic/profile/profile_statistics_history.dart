...
class ProfileStatisticsHistoryBloc
  ...
  ProfileStatisticsHistoryBloc() : super(ProfileStatisticsHistoryData()) {
    on<Reload>((data, emit) async {
      ...
      final r = await api.profileAdmin(
        (api) => api.getProfileStatisticsHistory(
      ...
      if (data.manualRefresh) {
        ...
        emit(state.copyWith(item: r.ok()));
      } else {
        emit(state.copyWith(isError: r.isErr(), isLoading: false, item: r.ok()));
      }
