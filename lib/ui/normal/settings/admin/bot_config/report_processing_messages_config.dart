...
class EditReportProcessingMessagesConfigPage
    extends MyScreenPageLimited<AdminBotReportProcessingMessagesConfig> {
  EditReportProcessingMessagesConfigPage(AdminBotReportProcessingMessagesConfig config)
    : super(builder: (closer) => EditReportProcessingMessagesConfigScreen(config, closer));
}

class EditReportProcessingMessagesConfigScreen extends StatefulWidget {
  final AdminBotReportProcessingMessagesConfig initialConfig;
  final PageCloser<AdminBotReportProcessingMessagesConfig> closer;
  const EditReportProcessingMessagesConfigScreen(this.initialConfig, this.closer, {super.key});

  @override
  State<EditReportProcessingMessagesConfigScreen> createState() =>
      _EditReportProcessingMessagesConfigScreenState();
}

class _EditReportProcessingMessagesConfigScreenState
    extends State<EditReportProcessingMessagesConfigScreen> {
  late AdminBotReportProcessingMessagesConfig _config;
  ...
  Widget _scaffold(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Report Messages Config")),
      body: Form(
        key: _formKey,
  ...
  Widget _llmEditor() {
    return Column(
      children: [
        ...
        TextFormField(
          initialValue: _config.llm.reportCreatorMessageTemplate,
          decoration: const InputDecoration(
            labelText: "Report Creator Message Template",
            helperText: "Placeholders: {text}, {message_number}",
          ),
          maxLines: null,
          validator: (value) => _config.llmEnabled ? _validateTemplateWithText(value) : null,
          onChanged: (v) {
            setState(() => _config.llm.reportCreatorMessageTemplate = v);
            _formKey.currentState?.validate();
          },
        ),
        TextFormField(
          initialValue: _config.llm.reportTargetMessageTemplate,
          decoration: const InputDecoration(
            labelText: "Report Target Message Template",
            helperText: "Placeholders: {text}, {message_number}",
          ),
          maxLines: null,
          validator: (value) => _config.llmEnabled ? _validateTemplateWithText(value) : null,
          onChanged: (v) {
            setState(() => _config.llm.reportTargetMessageTemplate = v);
            _formKey.currentState?.validate();
          },
        ),
        const Divider(),
  ...
  String? _validateTemplateWithText(String? value) {
